...
class Shape(_generic.DagNode):
    ...
    def component(self, name):
        ...
        # return
        return cgp_maya_utils.scene._api.component('{}.{}'.format(self.fullName(), name))
    ...
    def data(self, worldSpace=False):
        ...
        data['transform'] = self.transform().fullName()
    ...
    def duplicate(self, newTransform=False):
        ...
        newShape.attribute(self._INPUT_GEOMETRY).connect(source=self.attribute(self._OUTPUT_GEOMETRY))
        maya.cmds.refresh()  # refresh force de create connection to take effects
        newShape.attribute(self._INPUT_GEOMETRY).disconnect()
    ...
    def setTransform(self, transform, worldSpace=False, deleteOriginalTransform=False):
        ...
        # relative
        if not worldSpace:
            maya.cmds.parent(fullName, transform, shape=True, relative=True)
        ...
        # absolute
            ...
            maya.cmds.parent(fullName, tempLoc.fullName(), shape=True, relative=True)
    ...
    def transform(self):
        ...
        # get transform node
        xform = maya.cmds.listRelatives(self.fullName(), parent=True, fullPath=True)[0]
...
class NurbsCurve(Shape):
    ...
    def points(self):
        ...
        # return
        return ['{0}.cv[{1}]'.format(fullName, index) for index in range(self.count())]
    ...
    def points(self):
        ...
        for u in range(self.countU()):
            for v in range(self.countV()):
                data.append('{0}.cv[{1}][{2}]'.format(fullName, u, v))
...
class Mesh(Shape):
    ...
    def closestFace(self, positionX, positionY, positionZ):
        ...
        faceName = '{}.f[{}]'.format(self.fullName(), faceIndex)
    ...
    def closestVertex(self, positionX, positionY, positionZ):
        ...
        vertexName = "{}.vtx[{}]".format(self.fullName(), vertexDistances[min(vertexDistances.keys())])
    ...
    def export(self, name):
        ...
        # execute
        return cgp_generic_utils.files.createFile(filePath, content=[self.fullName()])
    ...
    def faceFromUV(self, uCoordinate, vCoordinate):
        ...
        for faceIndex in range(mesh.numPolygons):
            ...
            # return the face
            faceName = "{}.f[{}]".format(fullName, faceIndex)
    ...
    def points(self):
        ...
        # return
        return ['{0}.vtx[{1}]'.format(fullName, index) for index in range(self.count())]
