...
class GpuCache(_generic.DagNode):
    ...
    def delete(self):
        ...
        # execute
        maya.cmds.delete(maya.cmds.listRelatives(self.name(), parent=True, fullPath=True)[0])


class ShadingEngine(_generic.ObjectSet):
    """node object that manipulates a ``shadingEngine`` node
    """

    # ATTRIBUTES #

    _TYPE = cgp_maya_utils.constants.NodeType.SHADING_ENGINE

    # COMMANDS #

    def addMembers(self, nodes):
        """add nodes to the existing members of the ShadingEngine

        :param nodes: nodes to add as members
        :type nodes: list[str or :class:`cgp_maya_utils.scene.Node`]
        """

        # here we force the membership to handle the exclusivity constraint
        maya.cmds.sets(nodes, edit=True, forceElement=self.fullName())
