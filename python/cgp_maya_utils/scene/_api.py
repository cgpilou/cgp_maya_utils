...
def animKey(attr, frame, layer=None):
    ...
    attr = attribute(attr) if isinstance(attr, basestring) else attr
    layer = animLayer(layer) if isinstance(layer, basestring) else layer
...
def getAnimKeys(attributes=None,
    ...
    attributes = ([attribute(attr) if isinstance(attr, basestring) else attr for attr in attributes]
                  if attributes else [])
    ...
    # get keys from the animation layers
        ...
        # cast param in expected types
        animLayers = ([animLayer(layer) if isinstance(layer, basestring) else layer for layer in animLayers]
...
def animLayer(layer):
    ...
    # cast params in expected types
    layerNode = None if layer is None else node(layer) if isinstance(layer, basestring) else layer.node()
...
def attribute(fullName):
    ...
    # init
    genericType = cgp_maya_utils.constants.AttributeType.ATTRIBUTE

    # get type
    attributeType = maya.cmds.getAttr(fullName, type=True)

    # return
    return _ATTRIBUTE_TYPES.get(attributeType, _ATTRIBUTE_TYPES[genericType])(fullName)
